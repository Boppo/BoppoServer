...
function getImagesByEid()
{
  ...
  require $_SERVER['DOCUMENT_ROOT'] . '/BoppoServer/DBIO/UserImage.php';
...
function uploadImage()
{
  ...
  if (!file_exists("/var/www/html/boppo/Uploads/" . $uid))
    mkdir("/var/www/html/boppo/Uploads/" . $uid, 0777, true);
  if (!file_exists("/var/www/html/boppo/Uploads/" . $uid . "/" . $user_image_sequence))
    mkdir("/var/www/html/boppo/Uploads/" . $uid . "/" . $user_image_sequence, 0777, true);
  file_put_contents("/var/www/html/boppo/Uploads/" . $uid . "/" .
